...
#Oldest Julia version allowed to be registered
const minjuliaver = v"0.3.0"
#Oldest package version allowed to be registered
const minpkgver = v"0.0.1"
...
for (pkg, versions) in Pkg.Read.available()
    ...
    if ismatch(r"github\.com", host)
        ...
        #Check that newest version has a requires file and that it tags a
        #minimum Julia version that is at least the release version
        if !((pkg, maxv) in ( #List of grandfathered packages
            ...
            @assert maxv >= minpkgver "$pkg: version $maxv ( <$minpkgver ) no longer supported"
            ...
            open(requires_file) do f
                ...
                for line in eachline(f)
                   if startswith(line, "julia")
                       tokens = split(line)
                       @assert length(tokens)>1 "minimum julia version not specified"
                       juliaver = VersionNumber(tokens[2])
                       @assert juliaver ≥ minjuliaver "julia version $juliaver ( <$minjuliaver ) too old in $requires_file"
                       hasjuliaver = true
                   end
                end
                @assert hasjuliaver "no julia entry in $requires_file"
            end
        end
    end
end
